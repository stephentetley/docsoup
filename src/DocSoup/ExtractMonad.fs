...
module ExtractMonad = 
    ...
    // ****************************************************
    // Helpers for regex matching

    let internal genRegexIsMatch (getInput: unit -> ExtractMonad<'handle, string>) 
                                 (pattern:string) : ExtractMonad<'handle, bool> = 
        pipeM2 (getInput ())  
               (getRegexOptions ())
               (fun input regexOpts ->  
    ...
    let internal genRegexMatch (getInput: unit -> ExtractMonad<'handle, string>) 
        ...
        pipeM2 (getInput ())  
               (getRegexOptions ())
               (fun input regexOpts ->  
                    Regex.Match( input = input
                               , pattern = pattern
                               , options = regexOpts ))


    let internal genRegexMatchValue (getInput: unit -> ExtractMonad<'handle, string>) 
                            (pattern:string) : ExtractMonad<'handle, string> = 
        genRegexMatch getInput pattern >>= fun matchObj -> 
    ...
    let genRegexAllMatch (getInput: unit -> ExtractMonad<'handle, string>)
                         (patterns:string []) : ExtractMonad<'handle, bool> = 
        let predicates : ExtractMonad<'handle, bool> list = 
            patterns |> Array.toList |> List.map (genRegexIsMatch getInput)
        allM predicates


    let genRegexAnyMatch (getInput: unit -> ExtractMonad<'handle, string>)
                         (patterns:string []) : ExtractMonad<'handle, bool> = 
        let predicates : ExtractMonad<'handle, bool> list =  
            patterns |> Array.toList |> List.map (genRegexIsMatch getInput)
        anyM predicates